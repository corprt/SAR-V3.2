...
def generate_insights_gpt(temp_file_path,hf_embeddings):
    ...
    with st.spinner('Wait for it...'):
        if st.session_state.clicked1:
            ...
            Name can be identified from cardholder information \n\n\
                        Question: {query}\n\
                        Context: {context_1}\n\
                        Response: '''
            ...
            try:
                # resp_dict_obj = json.loads(res_dict)
    ...
    with st.spinner('Getting you information...'):      
        if query:
            ...
            if query.lower() == "what is the victim's name?":
                ...
                prompt_1 = f''' You need to act as a Financial analyst to identify the when the bank was notified of the Fraud i.e., the processing date. Given the context, provide a relevant and concise response.\n\n\
                            Question: {query}\n\
                            Context: {context_1}\n\
                            Response: '''
            ...
            # tmp_table_gpt = pd.concat([tmp_table_gpt, df], ignore_index=True)
            # tmp_table_gpt.drop_duplicates(subset=['Question'])

          

    return st.session_state["tmp_table_gpt"], st.session_state["sara_recommendation_gpt"], st.session_state["lineage_gpt"] 


# @st.cache_data(show_spinner=False)
# def summ_gpt_(tmp_table_gpt):
#     summ_dict_gpt = tmp_table_gpt.set_index('Question')['Answer'].to_dict()
...
def summarize_gpt(tmp_table_gpt):
    ...
    with st.spinner("Summarize...."):
        if st.session_state.clicked2:
            response_summ_gpt,summ_dict_gpt = summ_gpt_(tmp_table_gpt)
